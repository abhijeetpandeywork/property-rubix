<?php 
/** City view — shows locality cards. Click locality → locality page with projects. */
...
.locality-title { transition: color 0.3s ease; }
.explore-hint {
...
.premium-locality-card:hover .locality-title { color: var(--pr-primary) !important; }
.premium-locality-card:hover .badge { background: var(--pr-primary) !important; color: var(--pr-secondary) !important; }
.premium-locality-card:hover .explore-hint { opacity: 1 !important; transform: translateY(0); }
</style>
...
<div class="position-relative" style="height:55vh; min-height:450px; overflow:hidden; margin-top:-1px;">
  ...
  <div class="position-absolute top-0 start-0 w-100 h-100 d-flex flex-column justify-content-center align-items-center text-center px-3" style="z-index:2;">
    ...
    <p class="lead text-white-50 mb-0" style="max-width:700px;font-weight:400;font-size:1.15rem;line-height:1.6;">
      <?= number_format($totalProjects) ?> properties across neighborhoods in
...
<!-- Breadcrumb -->
<div class="glass-panel border-bottom sticky-top" style="z-index:10;top:76px;box-shadow:0 4px 20px rgba(0,0,0,.03);">
...
<div class="section py-5 position-relative" style="background:#f8f9fa;">
  <div class="container-fluid px-3 px-md-5">
    ...
    <!-- Search bar -->
    <div class="row justify-content-center mb-5" style="padding-top:15px;">
      <div class="col-lg-8 col-md-10">
        <div class="bg-white p-3 rounded-pill shadow-lg d-flex align-items-center" style="border:1px solid rgba(0,0,0,.05);">
          <i class="fas fa-search fs-4 text-muted ms-3 me-2"></i>
          <input type="text" id="localitySearch" class="form-control form-control-lg border-0 shadow-none bg-transparent"
                 placeholder="Find your perfect neighborhood..." style="font-size:1.1rem;">
        </div>
      </div>
    </div>
      ...
      <?php if (!empty($localities)): ?>
        <?php foreach ($localities as $loc): ?>
          <div class="col-xl-3 col-lg-4 col-md-6 locality-item-wrapper">
            <a href="<?= PUBLIC_URL ?>location/<?= e($city['country_slug']) ?>/<?= e($city['state_slug']) ?>/<?= e($city['slug']) ?>/<?= e($loc['slug']) ?>"
              ...
              <div class="locality-icon-wrapper mb-3">
                <i class="fas fa-map-marked-alt"></i>
              </div>

              <h3 class="fw-bold text-dark mb-2 locality-title" style="font-size:1.25rem;line-height:1.3;">
                <?= e($loc['location_area']) ?>
              </h3>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <!-- No localities? Show all city projects directly -->
        <div class="col-12 text-center py-5">
          <div class="mb-4 text-muted opacity-50"><i class="fas fa-building fa-4x"></i></div>
          <h4 class="fw-bold text-dark mb-2">No neighbourhoods listed yet.</h4>
          <p class="text-muted">We are actively adding exclusive projects in <?= e($city['name']) ?>.</p>
        </div>
      <?php endif; ?>
    </div>
...
document.getElementById('localitySearch')?.addEventListener('input', function (e) {
  ...
  document.querySelectorAll('.locality-item-wrapper').forEach(item => {
    const title = item.querySelector('.locality-title')?.innerText.toLowerCase() ?? '';
