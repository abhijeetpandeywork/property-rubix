...
for(let i=0; i<20; i++) {
    ...
    const connectivity = JSON.stringify({
        ...
        "Hospitals": "City Care Hospital, Metro Health Center",
        "Schools": "International School, Global Academy",
        "Malls": "Nexus Mall, City Center"
    });
    
    const highlights = "Premium Luxury Living<br>State of the art clubhouse<br>Olympic size swimming pool<br>Lush green landscapes<br>24/7 Security and Concierge";
    
    const description = "<p>Welcome to " + name + ", an ultra-premium residential project located in " + loc + ".</p>";

    sql += "INSERT INTO projects (builder_id, city_id, name, slug, type, status, price_min, price_max, price_on_request, unit_types, area_range, rera_id, rera_verified, address, location_area, short_description, description, banner_image, thumbnail_image, video_url, possession_date, is_featured, sort_order, meta_title, meta_description, project_logo, contact_phone, whatsapp_number, virtual_tour_url, rera_qr_code, connectivity, highlights, marquee_text, gallery_images, interior_images, exterior_images) VALUES ";
    sql += "(1, 1, '" + name + "', '" + slug + "', 'residential', 'under_construction', " + priceMin + ", " + priceMax + ", 0, '2, 3, 4 BHK', '1200 - 3500 Sq.Ft.', 'P123456789" + i + "', 1, '" + loc + "', '" + loc + "', 'Luxury apartments in " + loc + ".', '" + description + "', '" + banner + "', '" + banner + "', 'https://www.youtube.com/embed/tgbNymZ7vqY', 'Dec 2026', 1, " + i + ", '" + name + " | Luxury Homes', 'Buy " + name + " luxury homes in " + loc + ".', 'https://images.unsplash.com/photo-1599305445671-ac291c95aaa9?w=200&h=200&fit=crop', '+91 9999999999', '+91 9999999999', 'https://www.youtube.com/embed/tgbNymZ7vqY', 'https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=RERA123', '" + connectivity + "', '" + highlights + "', 'Special Offer!', '" + exteriors + "', '" + interiors + "', '" + exteriors + "');\n\n";
}
