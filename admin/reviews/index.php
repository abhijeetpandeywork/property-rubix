...
if ($action === 'delete' && $id) {
    csrfCheck();
    crudDelete($pdo, 'reviews', $id, BASE_URL . 'admin/reviews/');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['new','edit'])) {
    ...
    if (!$errors) {
        if ($id) {
            ...
            $stmt = $pdo->prepare("UPDATE `reviews` SET $sets WHERE id=?");
            $stmt->execute([...array_values($data), $id]);
            logAction('UPDATE', 'reviews', $id);
        } else {
            $cols = implode(', ', array_map(fn($k) => "`$k`", array_keys($data)));
            $vals = implode(', ', array_fill(0, count($data), '?'));
            $stmt = $pdo->prepare("INSERT INTO `reviews` ($cols) VALUES ($vals)");
            $stmt->execute(array_values($data));
            $id = (int)$pdo->lastInsertId();
            logAction('CREATE', 'reviews', $id);
        }
        header('Location: ' . BASE_URL . 'admin/reviews/?saved=1');
        exit;
    }
}

// ── List
$list = crudList($pdo, 'reviews', 20, 'id DESC', '', '', $search, ['id']);
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            <br><span class="text-muted small">Rating: <?= $p['rating'] ?? 5 ?>/5 | Status: <?= htmlspecialchars($p['status'] ?? 'pending') ?></span>
          </td>
          <td>
            <div class="actions">
  ...
  <div class="adm-card">
    <div class="row g-3">
        ...
        foreach ($columns as $col) {
            ...
            if ($c === 'status') {
                $statusVal = $row[$c] ?? 'pending';
                echo '<select name="'.$c.'" class="form-select">';
                echo '<option value="pending" '.($statusVal==='pending'?'selected':'').'>Pending</option>';
                echo '<option value="approved" '.($statusVal==='approved'?'selected':'').'>Approved</option>';
                echo '<option value="rejected" '.($statusVal==='rejected'?'selected':'').'>Rejected</option>';
                echo '</select>';
