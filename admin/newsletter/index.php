...
if ($action === 'delete' && $id) {
    csrfCheck();
    crudDelete($pdo, 'newsletters', $id, BASE_URL . 'admin/newsletter/');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['new','edit'])) {
    ...
    if (!$errors) {
        if ($id) {
            ...
            $stmt = $pdo->prepare("UPDATE `newsletters` SET $sets WHERE id=?");
            $stmt->execute([...array_values($data), $id]);
            logAction('UPDATE', 'newsletters', $id);
        } else {
            $cols = implode(', ', array_map(fn($k) => "`$k`", array_keys($data)));
            $vals = implode(', ', array_fill(0, count($data), '?'));
            $stmt = $pdo->prepare("INSERT INTO `newsletters` ($cols) VALUES ($vals)");
            $stmt->execute(array_values($data));
            $id = (int)$pdo->lastInsertId();
            logAction('CREATE', 'newsletters', $id);
        }
        header('Location: ' . BASE_URL . 'admin/newsletter/?saved=1');
        exit;
    }
}

// ── List
$list = crudList($pdo, 'newsletters', 20, 'id DESC', '', '', $search, ['id']);
