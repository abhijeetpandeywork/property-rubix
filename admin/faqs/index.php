...
if ($action === 'delete' && $id) {
    csrfCheck();
    crudDelete($pdo, 'faqs', $id, BASE_URL . 'admin/faqs/');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['new','edit'])) {
    ...
    if (!$errors) {
        if ($id) {
            ...
            $stmt = $pdo->prepare("UPDATE `faqs` SET $sets WHERE id=?");
            $stmt->execute([...array_values($data), $id]);
            logAction('UPDATE', 'faqs', $id);
        } else {
            $cols = implode(', ', array_map(fn($k) => "`$k`", array_keys($data)));
            $vals = implode(', ', array_fill(0, count($data), '?'));
            $stmt = $pdo->prepare("INSERT INTO `faqs` ($cols) VALUES ($vals)");
            $stmt->execute(array_values($data));
            $id = (int)$pdo->lastInsertId();
            logAction('CREATE', 'faqs', $id);
        }
        header('Location: ' . BASE_URL . 'admin/faqs/?saved=1');
        exit;
    }
}

// ── List
$list = crudList($pdo, 'faqs', 20, 'id DESC', '', '', $search, ['id']);
    ...
    <table class="adm-table">
      ...
      <tbody>
        ...
        <tr>
          <td><?= $p['id'] ?></td>
          <td>
            <strong><?= htmlspecialchars(mb_strimwidth($p['question'] ?? '', 0, 80, '...')) ?></strong>
            <br><span class="text-muted small">Status: <?= htmlspecialchars($p['status'] ?? 'active') ?></span>
          </td>
          <td>
            <div class="actions">
