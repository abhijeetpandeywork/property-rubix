...
if ($action === 'delete' && $id) {
    csrfCheck();
    crudDelete($pdo, 'subscribers', $id, BASE_URL . 'admin/subscribers/');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array($action, ['new','edit'])) {
    ...
    if (!$errors) {
        if ($id) {
            ...
            $stmt = $pdo->prepare("UPDATE `subscribers` SET $sets WHERE id=?");
            $stmt->execute([...array_values($data), $id]);
            logAction('UPDATE', 'subscribers', $id);
        } else {
            $cols = implode(', ', array_map(fn($k) => "`$k`", array_keys($data)));
            $vals = implode(', ', array_fill(0, count($data), '?'));
            $stmt = $pdo->prepare("INSERT INTO `subscribers` ($cols) VALUES ($vals)");
            $stmt->execute(array_values($data));
            $id = (int)$pdo->lastInsertId();
            logAction('CREATE', 'subscribers', $id);
        }
        header('Location: ' . BASE_URL . 'admin/subscribers/?saved=1');
        exit;
    }
}

// ── List
$list = crudList($pdo, 'subscribers', 20, 'id DESC', '', '', $search, ['id']);
